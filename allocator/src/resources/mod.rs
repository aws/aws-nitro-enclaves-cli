...
pub fn allocate_by_cpu_count(
    mut pool: Vec<ResourcePool>,
    target_numa: Option<usize>,
) -> Result<(), Error> {
    ...
    let total_cpu_count: usize = pool
        .iter()
        .map(|p| {
    ...
    for (numa_node, cpu_set) in cpu::find_suitable_cpu_sets(total_cpu_count)?
        .into_iter()
...
pub fn sanity_check_numa_nodes(cpu_set: &BTreeSet<usize>) -> Result<usize, Error> {
    //change the logic
...
fn allocate_cpus_n_pages(
    pool: &Vec<ResourcePool>,
    ...
    for enclave in pool {
        ...
        let huge_pages_allocation = match huge_pages::Allocation::new(numa_node, memory_mib) {
