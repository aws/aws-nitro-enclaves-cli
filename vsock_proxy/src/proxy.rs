...
impl Proxy {
    ...
    pub fn sock_accept(&mut self, listener: &VsockListener) -> VsockProxyResult<()> {
        ...
        let needs_resolve = |d: DateTime<Utc>, i: Duration| {
            (Utc::now() - d + Duration::seconds(2)) > i
        };

        if self.dns_resolve_date == None || needs_resolve(self.dns_resolve_date.unwrap(), self.dns_refresh_interval.unwrap()) {
