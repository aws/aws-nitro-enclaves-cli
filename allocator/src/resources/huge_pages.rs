...
fn configure_huge_pages(numa_node: usize, memory_mib: usize) -> Result<Pages, Error> {
    ...
    for page_size in get_huge_page_sizes(numa_node)?.into_iter().rev() {
        ...
        let huge_pages_path = format!(
            "{BASE_SYSFS_PATH}/node{numa_node}/hugepages/hugepages-{page_size}kB/nr_hugepages"
        );

        let current_pages: usize = std::fs::read_to_string(&huge_pages_path)?.trim().parse()?;
        let new_pages = current_pages + needed_pages;

        std::fs::write(&huge_pages_path, new_pages.to_string())?;

        let actual_pages: usize = std::fs::read_to_string(&huge_pages_path)?.trim().parse()?;
...
pub fn release_huge_pages(numa_node: usize, allocated_pages: &Pages) -> Result<(), Error> {
...
mod tests {
    ...
    impl TestContext {
        ...
        fn setup_node(&self, numa_node: usize) {
            fs::create_dir_all(format!("{}/node{}/hugepages", BASE_SYSFS_PATH, numa_node)).unwrap();
        }

        fn create_hugepage(&self, numa_node: usize, size: usize, content: &str) {
            let dir_path = format!(
                "{}/node{}/hugepages/hugepages-{}kB",
            ...
            assert!(
                ...
                "File wasn't created or disappeared immediately: {}",
                file_path
            );
        }
    ...
    fn test_get_hugepage_sizes() {
        ...
        //Single NUMA node
        {
            //x86_64
            {
                ...
                assert_eq!(
                    get_huge_page_sizes(0).unwrap(),
                    BTreeSet::from([1048576, 2048])
                );
            }
            //ARM
            {
                ...
                assert_eq!(
                    get_huge_page_sizes(3).unwrap(),
                    BTreeSet::from([1048576, 2048, 32768, 64])
                );
            }
        }
    }
