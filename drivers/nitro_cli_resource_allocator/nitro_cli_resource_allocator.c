...
#define NITRO_CLI_RESOURCE_ALLOCATOR_DEV_MINOR	246
#define NITRO_CLI_RESOURCE_ALLOCATOR_DEV_NAME	"nitro_cli_resource_allocator"

MODULE_AUTHOR("Amazon.com, Inc. or its affiliates");
MODULE_DESCRIPTION(NITRO_CLI_RESOURCE_ALLOCATOR_DEV_NAME);
MODULE_LICENSE("GPL");
...
static int nitro_cli_resource_allocator_open(struct inode *node,
					     struct file *file)
{
	return 0;
}
...
static long nitro_cli_resource_allocator_ioctl(struct file *file,
					       unsigned int cmd,
					       unsigned long arg)
{
	switch (cmd) {
...
static int nitro_cli_resource_allocator_release(struct inode *inode,
						struct file *file)
{
	return 0;
}

static const struct file_operations nitro_cli_resource_allocator_fops = {
	.open = nitro_cli_resource_allocator_open,
	.unlocked_ioctl = nitro_cli_resource_allocator_ioctl,
	.release = nitro_cli_resource_allocator_release,
};

static struct miscdevice nitro_cli_resource_allocator_miscdevice = {
	.minor  = NITRO_CLI_RESOURCE_ALLOCATOR_DEV_MINOR,
	.name   = NITRO_CLI_RESOURCE_ALLOCATOR_DEV_NAME,
	.fops   = &nitro_cli_resource_allocator_fops,
	.mode   = 0400,
};

static int __init nitro_cli_resource_allocator_init(void)
{
	...
	return misc_register(&nitro_cli_resource_allocator_miscdevice);
}

static void __exit nitro_cli_resource_allocator_exit(void)
{
	mutex_lock(&nitro_cli_resource_allocator_lock);
	...
	return misc_deregister(&nitro_cli_resource_allocator_miscdevice);
}

module_init(nitro_cli_resource_allocator_init);
module_exit(nitro_cli_resource_allocator_exit);
